...
class shape
{
    ...
    shape flatten(float const flatness) const
    {
        ...
        bool add_last_point = true)
    {
        ...
        if(dx*dx+dy*dy > flatness)
        {
            ...
            add_tesselated_curve(flatness, mx, my, (x1+x2)/2.0f, (y1+y2)/2.0f, x2, y2, add_last_point);
        }
...
int main(int argc, char const * argv[])
{
    ...
    auto const s_b = info.glyph_shape(info.glyph_index(0x262D));
